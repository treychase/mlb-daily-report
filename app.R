...
# Sources modular R/ files, then declares CSS, UI, and server.
# ═══════════════════════════════════════════════════════════════════════════════

source("R/globals.R")        # libraries, constants, %||%, safe(), pitch maps
source("R/data_fetch.R")     # all API / CSV fetch functions
source("R/data_process.R")   # parse_batting / parse_pitching / compute_monthly
source("R/ui_components.R")  # score_display, make_dt, modal builders
source("R/plots.R")          # create_movement_plot, create_strike_zone_plot
...
/* Force dark table colours even inside dark Bootstrap theme */
...
.btn-pitcher-profile-modal { background:linear-gradient(135deg,#1d3a6e,#1d4e89);
  border:1px solid #2d6cbe; color:#79b8ff; font-size:.82rem; padding:8px 20px;
  border-radius:8px; font-family:'DM Sans',sans-serif; font-weight:700; cursor:pointer;
  transition:all .2s; letter-spacing:.02em; }
.btn-pitcher-profile-modal:hover { background:linear-gradient(135deg,#1d4e89,#2563eb); color:#e6edf3; }
...
.pct-badge { width:42px; height:42px; border-radius:50%; display:flex; align-items:center;
...
/* Pitcher profile: pitch characteristic cards */
