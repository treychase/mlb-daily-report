...
# Density heatmap of pitch locations for a single pitch type, with strike-zone
# box + plate overlay.  Used by the tabbed pitcher profile view.
create_strike_zone_heatmap <- function(pitches_df, pitch_type) {
  dark_bg <- "#0d1117"
  no_data_plot <- function(msg)
    ggplot2::ggplot() +
    ggplot2::annotate("text", x = 0, y = 2, label = msg,
                      color = "#8b949e", size = 4.5) +
    ggplot2::xlim(-2, 2) + ggplot2::ylim(-0.4, 4.5) +
    ggplot2::theme_void() +
    ggplot2::theme(plot.background = ggplot2::element_rect(fill = dark_bg, color = NA))
  
  if (is.null(pitches_df) || !is.data.frame(pitches_df) || nrow(pitches_df) == 0)
    return(no_data_plot("No pitch-location data"))
  if (is.null(pitch_type) || is.na(pitch_type) || nchar(pitch_type) == 0)
    return(no_data_plot("Select a pitch type"))
  
  pd <- pitches_df[!is.na(pitches_df$pitch_type) &
                     toupper(as.character(pitches_df$pitch_type)) == toupper(pitch_type), ]
  pd <- pd[is.finite(pd$plate_x) & is.finite(pd$plate_z), ]
  pd <- pd[pd$plate_x > -3 & pd$plate_x < 3 & pd$plate_z > -1 & pd$plate_z < 5.5, ]
  
  if (nrow(pd) < 5)
    return(no_data_plot(paste0("Too few ", pitch_type, " pitches (", nrow(pd), ") for heatmap")))
  
  # Strike-zone box (rough MLB standard: ~17" wide, 1.5-3.5 ft vertical)
  sz_hw <- 17/24  # half-width in feet
  sz_lo <- 1.50; sz_hi <- 3.50
  ...
  ggplot2::ggplot(pd, ggplot2::aes(x = plate_x, y = plate_z)) +
    ggplot2::stat_density_2d_filled(contour_var = "ndensity", alpha = 0.90, bins = 8) +
    # Strike-zone box
    ggplot2::annotate("rect",
                      xmin = -sz_hw, xmax = sz_hw, ymin = sz_lo, ymax = sz_hi,
                      fill = NA, color = "white", linewidth = 1, linetype = "dashed") +
    # Plate
    ggplot2::geom_polygon(data = plate, ggplot2::aes(x = x, y = y),
                          fill = "#c9d1d9", color = "#e6edf3", linewidth = 0.8,
                          inherit.aes = FALSE) +
    # Blue → orange gradient matching the percentile bubbles
    ggplot2::scale_fill_manual(values = c(
      "#0d1117", "#1e3a8a", "#3b82f6", "#60a5fa",
      "#fbbf24", "#f97316", "#ea580c", "#dc2626")) +
    ggplot2::coord_fixed(ratio = 1, xlim = c(-2, 2), ylim = c(-0.4, 4.5),
                         expand = FALSE) +
    ggplot2::labs(
      x = "Horizontal Position (ft)", y = "Height (ft)",
      title = paste0(pitch_full_name(pitch_type), " (n=", nrow(pd), ")")
    ) +
    ggplot2::theme_minimal() +
    ggplot2::theme(
      ...
      axis.text   = ggplot2::element_text(color = "#8b949e", size = 8),
      axis.title  = ggplot2::element_text(color = "#6e7681", size = 9),
      plot.title  = ggplot2::element_text(color = "#e6edf3", size = 11,
                                          hjust = 0.5, face = "bold"),
      plot.margin = ggplot2::margin(10, 16, 8, 16)
    )
}