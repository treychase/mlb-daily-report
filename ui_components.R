...
build_player_modal_ui <- function(data) {
  ...
  pct_col <- function(v) dplyr::case_when(
  ...
  if (!isTRUE(data$is_pitcher)) {
    ...
    adj <- div(class="adj-stat-line",
      ...
      if(!is.null(war_h)&&!is.na(war_h))
        adj_stat(sprintf("%.1f", as.numeric(war_h)), "bWAR")
    )
    stats_grid <- tagList(
  ...
  # ════════════════════════════════════════════════════════════════════════
    ...
    stats_grid <- tagList(
      ...
      div(class="ms-stat-grid",
        ...
        sb(baa_v,"BAA"),  sb(fmt0(s$saves),"SV")
      )
    )
    war_p    <- data$war_val
    war_line <- if (!is.null(war_p) && !is.na(war_p)) {
      div(class="adj-stat-line",
        adj_stat(sprintf("%.1f", as.numeric(war_p)), "bWAR"))
    } else NULL

    div(style="padding:4px;",
      player_hdr,
      key_line,    # ERA / WHIP / K9 / BB9 / BAA slash line
      war_line,    # bWAR right after key stats (mirrors hitter OPS+/wRC+ row)
      div(style = "text-align:center; margin:10px 0 18px;",
        tags$button(
          class   = "btn-pitcher-profile-modal",
          onclick = sprintf(
            "Shiny.setInputValue('pitcher_profile_click',{id:%d},{priority:'event'})",
            as.integer(pid)
          ),
          HTML("\u26be\ufe0f\u2002 Pitcher Profile")
        )
      ),
...
build_pitcher_profile_ui <- function(data) {
  ...
  if (!is.null(ars) && is.data.frame(ars) && nrow(ars) > 0) {
    ...
    pcc <- function(v) dplyr::case_when(
      v >= 90 ~ "#ef4444", v >= 67 ~ "#f97316",
      v >= 34 ~ "#8b949e", v >= 11 ~ "#3b82f6", TRUE ~ "#1d4ed8")
    rv_bdg <- lapply(seq_len(nrow(ars)), function(i) {
      ...
      col    <- if (!is.null(pct)&&!is.na(pct)) pcc(as.integer(pct)) else "#8b949e"
      div(class="pct-item",
  ...
  if (!is.null(vl) || !is.null(vr)) {
    ...
    sp_row <- function(lbl, sp) {
      ...
      div(class="split-row",
        ...
        div(class="split-stat rate", g0(sp,"obp")),
        div(class="split-stat rate", g0(sp,"slg")),
        div(class="split-stat rate", f2(sp,"era")),
        div(class="split-stat rate", f2(sp,"whip")),
